...
class BookTaxiActivity : AppCompatActivity() {
    ...
    private fun saveTaxiData() {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("taxis/$uid\"")
        val taxiId = databaseReference.push().key // Generate a unique key for the taxi
        if (taxiId != null) {
            val taxi = Taxi(uid,taxiId, taxiNumber, driverName, address, phoneNumber)
            databaseReference.child(taxiId).setValue(taxi)
